    ...
    for i, line in enumerate(input_fh):
        ...
        # Inside-create-statement handling
            ...
            if line.startswith('"'):
                ...
                extra = re.sub("COMMENT [\w\d]+\s*", "", extra.replace("unsigned", ""))

                extra= extra.replace("COMMENT","")
                ...
                comment_lines.append('%s' % comment_line)
                name_lines.append('%s' % name)
            ...
            # Is it the end of the table?
                ...
                if len(comment) > 1:
                    ...
                    output.write("comment on table  \"%s\" is '%s';\n" % (current_table, new_comment))

                    for j, line in enumerate(name_lines):
                        print j
                        output.write("comment on column  \"%s\".\"%s\" is '%s';\n" % (current_table, line if j != (len(name_lines) - 1) else "", comment_lines[j]))
